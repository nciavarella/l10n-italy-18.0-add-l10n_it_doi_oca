...
class TestBillOfEntry(AccountTestInvoicingCommon):
    ...
    def test_storno_create(self):
        ...
        lengths = {"0": 1, "1": 2}
        invoice_line_ids = self.bill_of_entry.invoice_line_ids.filtered(
            lambda line: line.display_type == "product"
        )
        for index, boe_line in enumerate(invoice_line_ids):
            move_line_domain = [
            ...
            self.assertEqual(len(extraeu_expense_moveline), lengths[str(index)])
